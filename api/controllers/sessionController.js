...
const createSessionWithNumbers = async (req, res) => {
    const { date, status, numbers } = req.body;
    ...
    try {
        ...
        if (targetSession) {
            const numbersList = targetSession.numbers.map(num => num.dataValues.value);
        ...
        // Tạo các number
        const numbersToCreate = numbers.map(num => ({
            value: num,
            session_id: session.id,
        }));

        await Number.bulkCreate(numbersToCreate);

        res.status(201).json({
            message: 'Session and numbers created successfully',
            session,
        });
    } catch (error) {
